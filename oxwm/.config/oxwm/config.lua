...
local blocks = {
	...
	oxwm.bar.block.shell({
		format = "Vol: {}%",
		command = "amixer get Master | grep -oP '\\[\\d+%\\]' | head -1 | tr -d '[]%'",
		interval = 1,
		color = colors.green,
		underline = true,
		action = "kitty --class wiremix-float wiremix",
	}),
	oxwm.bar.block.static({
		text = "::",
		interval = 999999999,
	...
	oxwm.bar.block.datetime({
		format = "{}",
		-- %a (Day Name), %b (Month), %d (Date), %T (Time with seconds)
		date_format = "%a %b %d %T",
		interval = 1,
		color = colors.cyan,
		underline = true,
...
-- Launch Dmenu
oxwm.key.bind({ modkey }, "A", oxwm.spawn({ "rofi -show drun" }))
...
oxwm.key.bind({ modkey }, "V", oxwm.client.toggle_fullscreen())
oxwm.key.bind({ modkey }, "F", oxwm.client.toggle_floating())

-- Launch Yazi with custom class for floating rule
oxwm.key.bind({ modkey }, "E", oxwm.spawn({ "kitty", "--class", "yazi-float", "yazi" }))
oxwm.rule.add({ class = "kitty-notes", floating = true })
oxwm.rule.add({ class = "yazi-float", floating = true })
oxwm.rule.add({ class = "wiremix-float", floating = true })
-- Layout management
...
oxwm.key.bind({ ctrl }, "H", oxwm.set_master_factor(-5))
oxwm.key.bind({ ctrl }, "L", oxwm.set_master_factor(5))
...
oxwm.autostart("feh --bg-scale ~/Pictures/wallpapers/edgywallpapertokynonigh.png")
